...
const SOURCE_NAMES = new Set(Object.keys(ARTIFACTS));
...
function normalizeHumanDecision(question, decision, retrieved) {
  let gaps = (ARTIFACTS["gaps.json"].gaps || []).filter((gap) => gap.status === "open" && gap.resolution == null);
  const asksCityMatching = /(city|市区町村).*(照合|一致)|照合.*(city|市区町村)/i.test(question);
  if (asksCityMatching) gaps = gaps.filter((gap) => gap.id === "gap.city_matching_semantics");
...
function explicitEvidenceSource(question, route, retrieved) {
...
function enforceController(question, route, raw, retrieved, canRetrieveMore) {
  const decision = { ...raw };
  const warnings = [];
  const required = explicitEvidenceSource(question, route, retrieved);
...
function normalizeImpact(decision) {
  const warnings = [];
...
function completeCrossEvidence(decision, retrieved) {
...
function humanAuthority(route, question) {
  if (route !== "human_decision") return [];
  let gaps = (ARTIFACTS["gaps.json"].gaps || []).filter((gap) => gap.status === "open" && gap.resolution == null);
  if (/(city|市区町村).*(照合|一致)|照合.*(city|市区町村)/i.test(question)) {
    gaps = gaps.filter((gap) => gap.id === "gap.city_matching_semantics");
  }
  return gaps.map((gap) => ({
    gap_id: gap.id,
    topic: gap.question,
...
async function groundedStep(client, question, route, retrieved, canRetrieveMore) {
...
export async function runRetrieval(question, client) {
  ...
  try {
    ...
    const first = await groundedStep(client, question, route, retrieved, true);
    ...
    let { decision, warnings } = enforceController(question, route, first.result, retrieved, true);
    if (route === "impact_scope") {
      const normalized = normalizeImpact(decision);
    ...
    if (decision.sufficiency === "partial" && decision.additional_source && retrieved.length === 1) {
      ...
      const second = await groundedStep(client, question, route, retrieved, false);
      addUsage(usage, second);
      validateDecision(second.result, retrieved, false);
      modelSufficiency = second.result.sufficiency;
      const enforced = enforceController(question, route, second.result, retrieved, false);
      decision = enforced.decision;
      warnings.push(...enforced.warnings);
      completeCrossEvidence(decision, retrieved);
    }

    if (route === "human_decision") {
      decision = normalizeHumanDecision(question, decision, retrieved);
    }
    ...
    return {
      ...
      humanAuthority: humanAuthority(route, question),
